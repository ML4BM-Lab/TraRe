...
linkeroutput <- TraRe::LINKER_run(TraReObj, link_mode = "VBSR", module_rep = "LINKER",
                           ...
                           corrClustNrIter = 100) 
kk_lo <- LINKER_run_a(TraReObj, link_mode = "VBSR", module_rep = "LINKER",
                           graph_mode = "VBSR", NrModules = 100, Nr_bootstraps = 10,
                           corrClustNrIter = 100) 
# We save the output
...
prew_output <- TraRe::preparerewiring(name="Vignette_prueba", TraReObj = TraReObj,
...
```{r}
  fpath <- paste0(system.file('extdata',package='TraRe'),'/rewired_modules_50boots.txt')
# At the gene level (50 bootstraps)
  impgenes <-  rewiring_gene_level(linker_output = linkeroutput_50b,
                                   TraReObj = TraReObj,
                                   fpath = fpath,
                                   final_signif_thresh = 0.05,
                                   ImpTH = 0.05,
...
```{r}
  ...
  regulons <- rewiring_regulon_level(linker_output = linkeroutput_50b,
                                     TraReObj = TraReObj,
                                     sigmodules_p = fpath,
                                     final_signif_thresh = 0.05)
  saveRDS(regulons,paste0(system.file('extdata',package='TraRe'),
  '/regulons_example.txt'))

...
* **Phenotype dataframe**: an dataframe with the Sample IDs in the `rownames()` and a column named *phenotype* that codifies a **binary phenotype**. This variable will be converted to a factor with two levels and labels c(0,1), thus, unless specified the order of the factor levels will be set automatically. If this data is incorporated in a SummarizedExperiment object from the begining of the pipeline and not all samples have the phenotype information (NAs), NA's will be ommited at the rewiring test. 

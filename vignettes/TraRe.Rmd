...
# We have to make sure phenotype rows and lognorm_est_counts columns are the same.
...
```
Now we run the inference method
```{r}
...
prew_output <- TraRe::preparerewiring(name="Vignette_rewiring", TraReObj = TraReObj1,
...
# saveRDS(object = prew_output, file = paste0(system.file('extdata',package='TraRe'),
    #   '/VignetteFiles/preprew_quickexample_vignette.rds')
# Run rewiring
...
outdir <- system.file('extdata',package='TraRe')
prew_output <- TraRe::preparerewiring(name = 'Vignette_test', linker_output = linkeroutput,
                            TraReObj = TraReObj, 
...
- **fpath** Desired path for the rewiring file to be generated (or oath to existing file).
...
# At the gene level (50 bootstraps)
  impgenes <-  TraRe::rewiring_gene_level(linker_output = linkeroutput,
                                   TraReObj = TraReObj,
                                   fpath = fpath,
                                   final_signif_thresh = 0.05,
                                   ImpTH = 0.05,
...
```{r, eval = TRUE}
  ...
  regulons <- rewiring_regulon_level(linker_output = linkeroutput,
                                     TraReObj = TraReObj,
                                     sigmodules_p = fpath,
                                     final_signif_thresh = 0.05)

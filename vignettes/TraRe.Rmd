...
# saveRDS(object = SeObject, file = paste0(system.file("extdata",package = "TraRe"),
               # '/VignetteFiles/SeObject_quickexample_vignette.rds'))
...
# saveRDS(object = TraReObj, file = paste0(system.file("extdata",package = "TraRe"),
               # '/VignetteFiles/TraReObj_quickexample_vignette.rds'))
...
# We can save the output and import it later on using the following code:
saveRDS(object = linkeroutput,paste0(vignette_data_path,"/last_linkeroutput_50m10b.rds"))
# saveRDS(object = linkeroutput,
#         file = paste0(system.file('extdata', package = 'TraRe'),
#                      '/VignetteFiles/linkeroutput_quickexample_vignette2.rds'))
```
```{r,eval=TRUE,warning=FALSE,echo=FALSE}
linkeroutput <- readRDS(file = paste0(system.file('extdata', package = 'TraRe'),
                    '/VignetteFiles/linkeroutput_quickexample_vignette.rds'))
```
...
# We load gene expression matrix and convert it to matrix class
lognorm_est_counts_p <- paste0(system.file('extdata',package='TraRe'),
                              '/VignetteFiles/expression_rewiring_example.txt')
...
# saveRDS(object = TraReObj, file = paste0(system.file('extdata',package='TraRe'),
                          # '/VignetteFiles/TraReObj_rewiring_example.rds'))
```
```{r,eval=TRUE,warning=FALSE,echo=FALSE}
TraReObj <- readRDS(file = paste0(system.file('extdata', package = 'TraRe'),
              '/VignetteFiles/TraReObj_rewiring_example.rds'))
...
# For this vignette example and to save computing time the matrix we are using a subset
# of the original and we will be running only 1 bootstrap, but the results below
# will show a 10 bootstraps LINKER_run.
linkeroutput2 <- TraRe::LINKER_run(TraReObj = TraReObj, link_mode='VBSR',
                       graph_mode ='VBSR', module_rep= 'LINKER',
                       NrModules = 50, Nr_bootstraps = 10,
                       corrClustNrIter = 100, train_size = 0.8)

# We save it for later
# saveRDS(object = linkeroutput, file = paste0(system.file('extdata',package='TraRe'),
# 'VignetteFiles/linkeroutput_rewiring_example.rds'))
...
# phenotype_p <- paste0(system.file('extdata',package='TraRe'),
#                       '/VignetteFiles/phenotype_rewiring_example.txt')
# 
...
# 
# # outdir <- system.file('extdata',package='TraRe')
# prew_output <- TraRe::preparerewiring(name = 'Vignette_test_2nd', 
#                                       linker_output =linkeroutput,
#                                       TraReObj = TraReObj, 
#                                       final_signif_thresh = 0.05,
#                                       nrcores = 1, outdir = outdir)

# saveRDS(object = prew_output, file = paste0(system.file('extdata',package='TraRe'),
                           #   '/VignetteFiles/preprew_rewiring_example.rds')
...
# TraReObj <- TraRe::rewiring_add_phenotype(TraReObj, phenotype_35)
# 
# outdir <- system.file('extdata',package='TraRe')
# prew_output <- TraRe::preparerewiring(name = 'Vignette_test_2nd', 
...
method by default calculates all transcriptional modules that the hierarchical clustering finds except for the last one (which is composed by the rest of the modules that have little in common between each other) and generates all the graph objects and figures seen below and in the next sections.

![Rewiring Raw graph](igraphs.raw.graph.png){width=1500px,height=750px}
This is the raw graph for the transcriptional module identified. Targets are sorted based on a Z-score derived from a t-test with null hypothesis if samples separated by the selected condition are not deferentially expressed.
...
# At the gene level (50 bootstraps)
  ...
  print(impgenes)
# # Save
  # write.table(impgenes, file=paste0(outdir,'/table_rewired_tfs.tsv'), 
  #             row.names = TRUE, sep = "\t")
```
...
```{r, eval = F}
  fpath <- paste0(system.file('extdata', package='TraRe'),
                  '/VignetteFiles/rewired_modules_50boots_fs_0.05.txt')
  
  regulons <- rewiring_regulon_level(linker_output = linkeroutput2,
                                     TraReObj = TraReObj,
                                     sigmodules_p = fpath,
                                     final_signif_thresh = 0.05)
# saveRDS(regulons,paste0(system.file('extdata',package='TraRe'),
#   '/regulons_results_example.txt'))
...
#                                         '/expression_rewiring_vignette.txt'))
# 
...
# 
# drivers_n <- rownames(drivers[which(rownames(drivers)%in%R),])
# targets_n <- rownames(targets[which(rownames(targets)%in%P),])
# 

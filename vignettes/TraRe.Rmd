---
title: "TraRe: Identification of conditions dependant Gene Regulatory Networks???? lo cambiamos?"
...
* *Bayesian machinel learning model??*, a module-based approach to infer gene regulatory networks (GRN) from RNAseq data.
* *Phenotype-associated transcriptional rewiring*, for selecting disrupted modules subject to a phenotype condition.
* *Three-tier analysis of results*, for uncovering robust disruptions at the transcription factor (TF) level, at the regulon level and at the transcriptional module level.
* Visualization, for graphically displaying generated robust transcriptional rewired modules.
* Results, excel generation with useful information about TFs-target relationships inferred.
...
focusing on only ARSI patients (35 samples). Original data can be found in database of Genotypes and Phenotypes (dbGaP) with accession code phs000915.v2.p2 and the cBioPortal (RRID:SCR_014555) Public Datahub.  

The workflow to follow is:
  
* Run TraRe's inference method with the *linker_preprocessing* and `LINKER_run` functions on the available gene expression matrix (GEM), 1149 genes and 121 samples, to generate the modules governing the transcriptomic activity of all samples, where a module represents one or several related biological functions.

* Run TraRe's rewiring test with the *preparereriwring* and *runrewiring* functions by selecting the sample's (ARSI) phenotype (35 samples) to extract *LINKER* modules which expression can be separated very well according to the selected phenotype. Selected modules will be scored based on a hypergeometric test. This step automatically pulls out an html report for visualization and a directory with the results.

* Run TF and regulon level analysis with the *rewiring_gene_level* and *rewiring_regulon_level* functions.

## Data 

The dataset in this vignette is located at TraRe's package that can be found in the follwong GitHub repository (*https://github.com/ubioinformat/TraRe/tree/master/inst/extdata/VignetteFiles*).
The complete SU2C dataset contains about 13000 genes, but we will be using a subset in order to fulfill the space requirements of the package. 
We consider important to use a real dataset for a proper understanding of TraRe's results.

TraRe mainly require three types of gene-related files:

* **Gene expression matrix**: log-applied and MRM-normalized gene expression matrix, *lognorm_est_counts*.

* **Gene info dataframe**: two column dataframe with a Gene IDs column with the same annotations as the gene expression and
a boolean variable with '1's for the TF genes and '0's for the target genes. This variable will be called 'regulator'.

* **Phenotype dataframe**: two column dataframe with Sample IDs column and a 'Class' column codifying for
 Responders 'R' or NonResponders 'NR'. A boolean variable is also accepted.
...
If you desire to do the differential network analysis (i.e. full workflow with rewiring test), then phenotype info should be stored in the SummarizedExperiment::colData().
```{r, eval=FALSE}
...
TraReObj <- linker_preprocessing(data_matrix = SeObject)
 
# We run the GRN inference method 
linkeroutput <- LINKER_run(TraReObj, link_mode = "VBSR",
                           graph_mode = "VBSR", NrModules = 100, Nr_bootstraps = 10,
                           corrClustNrIter = 100)                        
# We save the output
saveRDS(object = linkeroutput, file = linker_output_p)

# Prepare data for rewiring

prew_output <- preparerewiring(name="Vignette", TraReObj = TraReObj,
                                linker_output= linker_output_p,
                                final_signif_thresh = 0.01, nrcores = 3,
                                outdir=outdir)
...
## How to run linker_processing and LINKER_run in TraRe

`linker_processing` is a helper funtion to preprocess the data (filter genes and samples with low variance) prior to the GRN inference process.
...
- **geneinfo**: array of the regulator gene names that are in the specified data matrix.
- **nassay**: parameter to specify the name of the assay containing the desired matrix in the SummarizedExperiment object. Default: 1 (first item in assay's list).
- **low_var_genes_th**: Perform a filtering below this threschold to drop out low variance genes across samples. Default: 0.25
- **low_var_samples_th**: Perform a filtering below this threschold to drop out low variance samples across genes. Default: 1

The return object is a **TraReObj** containing preprocessed input matrix to pass to `LINKER_run`
```{r, eval=FALSE}
...
TraReObj <- linker_preprocessing(data_matrix = lognorm_est_counts,
                                 geneinfo = rownames(drivers), verbose = FALSE)

# We save it for later
# saveRDS(TraReObj, paste0(system.file('extdata',package='TraRe'),'/TraReObj.rds'))
```
...
# For this vignette example and to save computing time the matrix we are using is a subset of the original and we will be running only 1 bootstrap, but the results below
...
on our generated GRN. Thus, for each GRN generated in the previous step the difference of the  gene-gene  covariance  matrix 
associated with each binary-labeled phenotype is computed separately, namely 𝛴଴ and 𝛴ଵ. Then, the 
distance between the two covariance matrices is computed as the Frobenius norm of the difference 
between both of them. This distance 𝑠, which we term the rewiring score, is used as a statistic to measure the dissimilarity 
between the  covariance  matrices, and thus, the differential transcriptional mechanisms of the modules across phenotypes. 
A module is said to be rewired if its rewiring score 𝑠 satisfies 𝑃ுబሺ𝑠ሻ<𝜏, where 𝜏 is a user-defined 
threshold. A significant dissimilarity over the covariance matrices may indicate fundamental underlying disruptions in gene co-expression between phenotypes.
...
As bootstrapping has been performed in `LINKER_run`, this step will take advantage of it and will try to group (using hierarchical clustering) highly scored modules, to infer GRNs with similar behavior across bootstraps. 
...
In order to run do the differential network analysis, TraRe provides two functions: the `preparerewiring()` function and the `runrewiring()` function. 
The first one requires gathers all the data in an object to be used by the second. 
...
## Add the phenotype if it was not in the SummarizedExperiment when we did the preprocessing using hte helper function `rewiring_add_phenotype`
TraReObj <- rewiring_add_phenotype(TraReObj, phenotype)

outdir <- system.file('extdata',package='TraRe')
prepared <- preparerewiring(name = 'Vignette', linker_output = linker_output,
                            TraReObj = TraReObj, 
...
It will create a folder on the specified output path with an html report containing the hierarchical clustering of the rewired modules in the form of a heatmap, and a report containing statistical information about the performed test.

![Rewiring Dendogram](mod_sim.VBSR.1.heatm.png){width=2400px,height=2400px}
As shown in the heatmap, there a is a clear rewired and robust **Transcriptional Module** (TM). For a TM to exist
there are some requirements that have to be fulfilled. First, these modules should contain genes whose expression can be separated very well using the ARSI phenotype (*rewired* modules).
Second, they have shown similar behavior across bootstraps, with a score
given by its hypergeometrical test. Therefore, in this example there is one TM formed by the modules number `c(298,945,698,619,795,156,63,214,563)`.

![Rewiring Hierarchical Clustering](mod_sim.VBSR.1.dendro.png){width=2400px,height=1200px}

In the figure above is the Hierarchical Clustering representation of the TM. The current implementation of *Rewiring*
method pulls away all but the last TM by default and generates all the graph objects and figures we are showing in the next section.


### Add here the graph plots
and som eexplanation.
...
function `rewiring_gene_level`. We propose to use it using a liker_output generated with 50 bootstraps to increase the power of the test. The function first performes
the *Rewiring test* and then for each TF it computes the Fisher's test to discriminate whether this TF is more likely be found in rewired modules or not.
These are the main parameters of the function:
\*Note change to acomodate to new objects and files in LINKER_run and preparereriwring
...
2. The list of rewired TF with the p-value obtained in the test and the OR.
...
```
# Rewiring at the regulon level (50 bootstraps)

TraRe also provides a function to extract from rewired modules the curated rewired regulon for each TF. We again recomend to run it using a 50-bootstraps linker output for robustness.
Following are the main parameters for the `rewiring_regulon_level` function:
\*Note change to acomodate to new objects and files in LINKER_run and preparereriwring
...
The output is a list of regulons, this is, for each TF with significant results a dataframe describing the regulon information: target, p-value for this edge and multiplicity value (i.e. the numebr of times this edge has been found)
```{r, eval = FALSE}
...
## Overview
TraRe also provides a way of manually generating a single GRN from specified list of driver and target genes. 
Normally we would use the cluster of modules which behave similarly across bootstraps detected in the *Rewiring* step.
This eases the task of analyzing relationships between drivers and targets by constraining all the provided genes to a single GRN using `NET_Run()`.
Additionally TraRe provides a graphical way of detecting condition-dependent GRN deregulation on the selected rewired modules. 

...
prostate cancer data, containing 12971 genes (~= 2000 transcription factors or driver 
...
focusing on only ABI patients (22). The pathway we will follow is:
  
  * Run LINKER using the complete data, 12971 genes and 39 samples, to generate 
  all possible modules (GRN).
  * Run Rewiring test selecting the phenotype of ABI (22 samples) to extract 
  LINKER modules that, its expression can be separated very well according to
  ABI and behave similar across bootstraps.
  * Run Visualization, generating GRN graphs of how the regulation process may 
  change depending on the phenotype condition.
  * Run Results, that generates an excel containing information about the driver-target
  relationships from previous generated GRN graphs, and a brief summary that may be 
  useful for biological validation of GRN.

...
* Run *LINKER* using the available gene expression matrix (GEM), 1200 genes and 121 samples, to generate all possible modules (GRN).
...
TraRe mainly require three type of gene-related files:
...
# For this example, we are going to join 200 drivers and 1000 targets from our package's folder.
...
regulator_filtered_idx <- seq_len(R) #1:200
target_filtered_idx <- R+seq(P) #201:1200
...
geneinfo <- geneinfo[rownames(lognorm_est_counts),,drop=F]
...
# will show a 10 bootstrap LINKER run.
...
# In order to prepare the rewiring, we can provide individual files path:
...
prepared <- preparerewiring(name="Vignette",linker_output_p = linker_output_p,
                           SEObject_p = PrepareSEObject_p,final_signif_thresh=0.05)
...
# to provide a vector of paths that a complex structure of files.
...
drivers <- lognorm_est_counts[seq(200),]
targets <- lognorm_est_counts[200+seq(1000),]

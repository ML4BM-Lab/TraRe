...
For fully understanding of how TraRe works and the tools
it contains, we will go through every one of it: LINKER, a way
of inferring gene regulatory networks, Rewiring, for selecting 
important modules from LINKER subject to a phenotype condition, Visualization, 
for graphically displaying generated GRN from previous process and Results,
excel generation of useful information about drivers to targets relations from
previous GRN.

Along with every step description and explanation, we will be working with a SU2C (Stand Up 2 Cancer)
prostate cancer data, containing 12971 genes ($\approx$ 2000 transcription factors or driver 
genes) and 39 filtered samples. Patients from this clinical case were treated with 
Abiraterone (ABI) and Enzalutemide. For the phenotype dependant rewiring, we will be 
focusing on only ABI patients (21). The pathway we will follow is:
  
* Run LINKER using the complete data, 12971 genes and 39 samples, to generate all possible modules (GRN).

* Run Rewiring test selecting the phenotype of ABI (21 samples) to extract LINKER modules that, its expression can be separated very well according to ABI and behave similar across bootstraps.

* Run Visualization, generating GRN graphs of how the regulation process may change depending on the phenotype condition.

* Run Results, that generates an excel containing information about the driver-target relationships from previous generated GRN graphs, and a brief summary that may be useful for biological validation of GRN.
...
to fulfill the space requirements of the package. 
...
the pathway, running *LINKER* using the complete dataset to generate
all possible Gene Regulatory Networks.
...
a look at `preparerewiring()` function for more information. We now generate 
the `preparerewiring()` output.


Note that here we will include the previous generated *LINKER* output. 
You can find all these files at **jesusdfc/TraRe** repository.
...
![Rewiring Dendogram](C:/Users/jdelafuentec/Dropbox/PhD/TraRe/vignettes/mod_sim.VBSR.heatm.png){width=2400px,height=2400px}
As shown in the heatmap, there a is a clear **Super Module** (SM). For a SM to exist
, there are some requirements that have to be fulfilled within module's SM. This
modules contains genes whose expression can be separated very well using the ABI phenotype.
A part from these, they have shown a similar behavior across bootstraps, with an score
given by its hypergeometrical test. So these modules formed by `c(298,945,698,619,795,156,63,214,563)`
are forming this SM. 

![Rewiring Hierarchical Clustering](C:/Users/jdelafuentec/Dropbox/PhD/TraRe/vignettes/mod_sim.VBSR.dendro.png){width=2400px,height=1200px}
...
method extract the first SM it finds and generate all the graph objects 
...
![Rewiring Hierarchical Clustering](C:/Users/jdelafuentec/Dropbox/PhD/TraRe/vignettes/igraphs.refined.graphs.png){width=1500px,height=750px}
...
* *Cliques:* From the chosen individual GRN, an excel file is generated containing drivers-targets relationships and cliques. The way LINKER method works can make some highly-correlated driver genes (TFs) may be dropped from the resultant model, as the role they play at the GRN inference process is very similar. Due to this,
we propose a method based on cliques (Fully Connected Networks) to recover those dropped drivers.

From all previous analysis, we provide an informative way of looking at the generated GRN, the relationships they have within the network, easing a possible biological validation afterwards this in silico analysis.

# Generate file in TraRe

The `excel_generation()` function takes as input a working path and a boolean variable,
specifying if is desirable to include cliques in the summary of the generated excel. If so,
...
![Excel Generation](C:/Users/jdelafuentec/Dropbox/PhD/TraRe/vignettes/Excel.png){width=650 height=200px}

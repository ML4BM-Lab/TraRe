...
rewiring_test = function(x, grp, perm = 500) {
  ...
  if(sum(bools)<p){
    methods::show(paste(collapse = " ", c("... Rewiring Test - Dropping 0 Variance Genes:",
                                          paste(collapse=",",names(which(bools==FALSE))))))
  ...
  ## test stat
  Tval <- sum(apply(utils::combn(numgrp, 2), 2, function(pair) {
  ...
  pval = mean(c(Tval, T_star) >= Tval, na.rm=TRUE)
  return(list(pval=pval, T_star=T_star, Tval=Tval))
}

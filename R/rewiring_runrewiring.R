...
runrewiring <- function(ObjectList) {
    ...
    for (dupla in duplas) {
        ...
        # Generate heatmap
        gen_heatmap <- function(ObjectList, module_membership_list, allstats, cmp = FALSE, modmeth = "", i=0) {
        ...
        clusters <- gen_heatmap(ObjectList, module_membership_list, allstats, modmeth = modmeth, i = i)
    ...
    if (length(duplas) > 1) {
        ...
        gen_heatmap(ObjectList, c_module_membership_list, c_allstats, TRUE, "combined",i+1)
    }
}

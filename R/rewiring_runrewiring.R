...
runrewiring <- function(ObjectList) {
    ...
    statsnames <- c("module-method", "module-index", "orig-pval", "revised-pvalue", "num-targets", "num-regulators", "regulator-names", "target-names",
        "num-samples", "num-genes", "num-class1", "num-class2")
    ...
    for (dupla in duplas) {
        ...
        GenerateStats <- function(mymod) {
            ...
            new_pval <- orig_pval
            stats <- c(modmeth_i_c, mymod, signif(orig_pval, 3), signif(new_pval, 3), length(modtargs), length(modregs), regnames, targnames,
                dim(modmat), class_counts)
        ...
        gen_heatmap <- function(ObjectList, module_membership_list, allstats, cmp = FALSE, modmeth_i = "") {
            ...
            fisher_tbl <- NULL
            fisher_cols <- c("user_gene_set", "property_gene_set", "universe_count", "user_count", "property_count", "overlap_count", "pval")
            ...
            for (pair_idx in seq_len(nrow(module_pairs))) {
                ...
                stats <- c(module_pairs[pair_idx, ][1], module_pairs[pair_idx, ][2], universe_size, length(mod1genes), length(mod2genes), length(intersect(mod1genes,
                  mod2genes)))
                # show(stats)

                # building contigency table.
                contig_tbl <- as.table(matrix(c(length(intersect(mod1genes, mod2genes)), length(setdiff(mod1genes, mod2genes)), length(setdiff(mod2genes,
                  mod1genes)), universe_size - length(mod2genes) - length(mod1genes) + length(intersect(mod1genes, mod2genes))), ncol = 2,
                  byrow = TRUE))
                # show(contig_tbl)
            ...
            if (length(all_modules) > 1) {
                ...
                gplots::heatmap.2(heatm, Rowv = FALSE, Colv = FALSE, scale = "none", col = my_palette, breaks = my_col_breaks, dendrogram = "none",
                  cellnote = round(heatm, 0), trace = "none", density.info = "none", notecol = "black", margins = c(8, 8), cexRow = 1.5, cexCol = 1.5,
                  lmat = rbind(c(4, 4), c(2, 1), c(0, 3)), lwid = c(1, 4), lhei = c(1, 8, 0.1))
                grDevices::dev.off()
                # write plot to index page
                write(paste0("<img src='", indexpageinfo$imgstr, myplotname, ".dendro.png", "' alt='", myplotname, "' height='", 300, "' width='",
                  600, "'> &emsp; <br>\n"), paste0(indexpageinfo$htmldir, indexpageinfo$indexpath), append = TRUE)
                write(paste0("<img src='", indexpageinfo$imgstr, myplotname, ".heatm.png", "' alt='", myplotname, "' height='", 600, "' width='",
                  600, "'> &emsp; <br>\n"), paste0(indexpageinfo$htmldir, indexpageinfo$indexpath), append = TRUE)
            ...
            write_tables_all(allstats[sortidxs, ], tabletype = paste0(modmeth_i, "_mod_rewiring_scores"), filestr = "data", html_idxs = seq_len(nrow(allstats)),
            ...
            write_tables_all(fisher_tbl[sortidxs, ], tabletype = paste0(modmeth_i, "_sigmod_overlap"), filestr = "data", html_idxs = seq_len(nrow(fisher_tbl)),
                htmlinfo = indexpageinfo)
        ...
        c_module_membership_list <- c(c_module_membership_list, module_membership_list)

        # Check if more than one dataset is being analyzed, in which case only heatmap is available, so program finish here.
        ...
        for (numclus in utils::head(seq_along(clusters$clusters), -1)) {
            ...
            foldername_p <- paste0("supermodule_", numclus, "_", modmeth, "_" ,i)
            ...
            rawsummary <- function(cut = FALSE) {
              ...
              write_tables_all(rawsumm$fulledgesumm[sortidxs, ], tabletype = paste0(modmeth, "_raw_edgesumm"), filestr = "data", html_idxs = seq_len(nrow(rawsumm$fulledgesumm)),
                htmlinfo = indexpageinfo, extradir = paste0(foldername_p, "/"), glossarypath = "../glossary.html")
            ...
            # If we have failed generating the raw full graph, we cant generate
            # refined graphs so we generate the summaries without the graphs and we
            # finish.

            if (rawsumm$cut)
              return(rawsummary(TRUE))

            rawsummary()
            refinedmod <- unique(names(igraph::V(rawsumm$full_graph)))
            refinedrunmoddata <- list(regulators = refinedmod[(gene_info_df_keep[refinedmod, regulator_info_col_name] == "1")],
                                      target_genes = refinedmod[(gene_info_df_keep[refinedmod,
            ...
            mylayout <- return_layout_phenotype(refinedrunmoddata$regulators, refinedrunmoddata$target_genes, refinedsumm$nodesumm, rownames(norm_expr_mat_keep))
            ...
            write_tables_all(refinedsumm$nodesumm[sortidxs, ], tabletype = paste0(modmeth, "_refined_nodesumm"), filestr = "data", html_idxs = seq_len(nrow(refinedsumm$nodesumm)),
              htmlinfo = indexpageinfo, extradir = paste0(foldername_p, "/"))
            ...
            html_from_graph(gpath = paste0(outdir, "/", foldername_p, "/refinedsumm.rds"),
                            wpath = paste0(outdir, "/", foldername_p), user_mode = FALSE,
                            dataset = norm_expr_mat_keep[allregs, ])

            },error = function(x){
              message('Html summary could not be generated')
              message(x)
            })

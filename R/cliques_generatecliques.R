...
generatecliques<-function(dataset=NULL,nassay=1,method="pearson",correlationth=0.6,sparsecorrmatrix=TRUE,numcliques='All',mandatorygenes=c(),selection=1){ #All but the last one to print it in a different way.
  ...
  #ml-> cliques.
  #We assure max variance genes are getting in.
  sortparameter<-vapply(ml[],getMaxVariance,dat=pdobject,FUN.VALUE=1.0)
  ...
  x_axis<-vapply(ml[],getAvgVariance,dat=pdobject,FUN.VALUE = 1.0)
  y_axis<-vapply(ml[],getMedianCorrelation,dat=pdobject,FUN.VALUE = 1.0) #Median Correlation
  ...
  for (i in seq_along(ml)){
    ...
    ch_cat<-which(c((clique_l==1)&(sortparameter_ix_numcliques[i]<smobject$L),
                    (clique_l>1)&(clique_l<=4),(clique_l>4)&(clique_l<=7),
                    (clique_l>7),(sortparameter_ix_numcliques[i]>=smobject$L)))

    col_fact<-c(col_fact,switch(ch_cat,"x == 1","1 < x <= 4",
                                "4 < x <= 7","x > 7","Singleton"))
  ...
  SelectedCliquesPlot<-ggplot2::ggplot()+
    ...
    ggplot2::labs(
      ...
      subtitle = paste("TotalNum: ",toString(sum(mll)),
                       ...
                       toString(as.double(table(vapply(ml,length,FUN.VALUE = 1)==1)[2])),
                       ", Number of Cliques: ",
                       toString(numcliques),
                       sep="")
    )+
    ...
    ggplot2::theme(legend.text = ggplot2::element_text(size = 15),
                   axis.text=ggplot2::element_text(size=15),axis.title = ggplot2::element_text(size=15),
                   legend.title = ggplot2::element_text(size = 15))+

    ggplot2::scale_color_manual(name="Genes/Cliques",
                                values = c("x == 1"=Palette[1],
                                           "1 < x <= 4"=Palette[2],"4 < x <= 7"=Palette[3],
                                           "x > 7"=Palette[4],"Singleton"=Palette[5]))

  return(list(plot=SelectedCliquesPlot,
              cliques=RC_list,
              representatives=names(RC_list)))
}
#' @export
...
generategraph<-function(correlationth,sparsecorrmatrix,pdoutput){
  ...
  if (sparsecorrmatrix){highcorg_g[highcorg_g<correlationth]<-0}
...
selectmethod <- function(selection,ggoutput,pdoutput){
  ...
  SortingMethod_chosen<-vapply(Cliques,SortingMethod,FUN.VALUE = 1.0)
...
getMaxVariance <- function(clique,dat){return (max(vapply(unlist(clique),function(x) dat$hash[[x]],FUN.VALUE = 1.0)))}

getMaxVarName <- function(clique,dat){return (names(which.max(vapply(unlist(clique),function(x) dat$hash[[x]],FUN.VALUE = 1.0))))}

getAvgVariance<-function(clique,dat){ return (mean(vapply(clique,function(x) dat$hash[[x]],FUN.VALUE = 1.0)))}

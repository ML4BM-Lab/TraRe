...
heatmapplot <- function(heatm, plotname = "", myzlim = c(min(heatm), max(heatm)), cvec = c("red", "white", "blue"), showRows = T) {
    ...
    if (showRows) {
        if (ncol(heatm) > 10) {
            ...
            graphics::axis(2, at = idxs, labels = rownames(heatm)[idxs], las = 2, cex.axis = 0.8, tick = F, col.axis = "black")
        } else {
            graphics::axis(2, at = 1:dim(heatm)[1], labels = rownames(heatm), las = 2, cex.axis = 0.8, tick = F, col.axis = "black")
        }
    }
}
#' @export
...
plot_expression_row <- function(mymat = NULL, rowdesc = "Regulators", plotheight = 200, myshowrows = TRUE, samps2pheno = NULL, phenostrs = c("nonrespond", 
    ...
    for (respstr in phenostrs) {
        ...
        heatmapplot(mymat[, names(which(samps2pheno == respstr)), drop = F], plotname = titlename, myzlim = plotzlim, cvec = mycvec, 
            showRows = myshowrows)
        grDevices::dev.off()
...
plot_correlation_row <- function(cormats = NULL, rowdesc = "regulators", xnames = NULL, ynames = NULL, plotheight = 200, myshowrows = TRUE, 
    ...
    for (mattype in ls(cormats)) {
        ...
        # show(myplotname)
        grDevices::png(paste0(imgdir, myplotname, ".png"), width = plotwidth, height = plotheight)
        heatmapplot(cormats[[mattype]][xnames, ynames, drop = F], plotname = titlename, myzlim = plotzlim, cvec = mycvec, showRows = myshowrows)
        grDevices::dev.off()
        write(paste0("<td> <img src='", "imgs/", myplotname, ".png", "' alt='", myplotname, "' height='", plotheight, "' width='", 
            plotwidth, "'> </td>\n"), htmlfile, append = T)
    }

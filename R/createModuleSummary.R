createModuleSummary <- function(ObjectList,modmeth="VBSR", numclus=1, supertype="refined") {
	# modmeths <- names(ObjectList$datasets[[1]]$rundata$modules)
	...
	if (file.exists(modsumm_name) && file.exists(runmoddata_name)){
		modsumm <- readRDS(modsumm_name)
	} else {
        stop("Rewiring files does not exist, please run runrewiring before creating module summary.")
	}
	...
	# bipartiteGraphsSumm(numclus, modsumm, ObjectList$datasets[[1]]$norm_expr_mat_keep, nonrespond_idxs, responder_idxs, modmeth, htmlinfo)

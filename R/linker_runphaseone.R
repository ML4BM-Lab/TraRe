...
LINKER_ReassignGenesToClusters <- function(Data, RegulatorData, Beta, Clusters, NrCores = 1) {
    ...
    nc <- BiocParallel::bplapply(seq_len(NrGenes),ReassignGenes,
                                 Data,BPPARAM = parallClass)
...
LINKER_LearnRegulatoryPrograms <- function(Data, Clusters, RegulatorData, Lambda,
                                           alpha, pmax, mode, used_method = "MEAN",
                                           NrCores = 1)
    {
    ...
    #Calculate Bettas
    CalcBettas <- function(i,Data,Clusters,RegulatorData,
                           Lambda,alpha,mode,used_method)
        {
    ...
    #Initialize
    BetaY_all <- BiocParallel::bplapply(seq_len(NrClusters),CalcBettas,Data,
                                        Clusters,RegulatorData,Lambda,alpha,
                                        mode,used_method,
                                        BPPARAM = parallClass)

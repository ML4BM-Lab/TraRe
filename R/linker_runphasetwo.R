...
LINKER_runPhase2 <- function(modules, Data, NrCores, mode = "VBSR", alpha = 1 - 1e-06) {
    ...
    runPhase2Bettas <- function(mod_idx){
        ...
        if (length(regulators) < 2) {
            ...
            non_zero_beta <- modules[[mod_idx]]$regulatory_program[which(modules[[mod_idx]]$regulatory_program !=
                                                                             0)]
            ...
            for (idx_gene in seq_along(targetgenes)) {
                ...
                if (mode == "VBSR") {
                    ...
                    for (idx_regs in seq_along(regulators)) {
                        ...
                        fit <- stats::lm(y ~ x)
                        s <- summary(fit)
                        driverMat[idx_gene, idx_regs] <- s$coefficients[2, "Pr(>|t|)"] < 0.05/(length(targetgenes) *
                                                                                                   length(regulators))
                    }
                } else {

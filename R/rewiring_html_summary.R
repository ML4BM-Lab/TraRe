...
module_node_summary <- function(norm_mat_keep, runmoddata, name2idx, nonrespond_idxs, responder_idxs){
  ...
  for (gene in genenames){
    ...
    myrow = c(signif(tres$p.value, 3), signif(tres$statistic, 3),
              signif(wres$p.value, 3), signif(wres$statistic, 3))
    test_vals = rbind(test_vals, myrow)
  }
  nodesumm = cbind(genenames, isreg,
                   signif(avgAll,3), signif(avg0,3), signif(avg1,3), signif(avgdiff,3),
                   test_vals, repAll, rep0, rep1)
  colnames(nodesumm) = c("gene-name", "is-regulator",
                         'avg-all', 'avg-nonresp', 'avg-respond', 'avg-diff',
                         't-pval', 't-stat',
                         'wilcox-pval', 'wilcox-stat',
                         'num-repeats-all', 'num-repeats-nonresp', 'num-repeats-respond'
  )
...
module_edge_summary <- function(norm_mat_keep, runmoddata, name2idx,nonrespond_idxs,responder_idxs){
  ...
  for (myreg in runmoddata$regulators){
    for (mytar in runmoddata$target_genes){
      ...
      myrow = c(paste0(mytar,"||",myreg),myreg,mytar,
                pcorall$estimate, pcorall$p.value, pcor0$estimate, pcor0$p.value, pcor1$estimate, pcor1$p.value,
                spcorall$estimate, spcorall$p.value, spcor0$estimate, spcor0$p.value, spcor1$estimate, spcor1$p.value)
      edgesumm = rbind(edgesumm, myrow)
    }
  }
  colnames(edgesumm) = c("key", "reg", 'target',
                         'all-pearson', 'all-pearson-pvalue','nonresp-pearson', 'nonresp-pearson-pvalue','respond-pearson', 'respond-pearson-pvalue',
                         'all-spearman', 'all-spearman-pvalue','nonresp-spearman', 'nonresp-spearman-pvalue','respond-spearman', 'respond-spearman-pvalue')
...
summarize_module<-function(norm_expr_mat_keep, runmoddata, name2idx, nonrespond_idxs, responder_idxs)
{
  ...
  # node summary
  nodesumm = module_node_summary(norm_expr_mat_keep,
                                 runmoddata,
                                 name2idx,
                                 nonrespond_idxs,
                                 responder_idxs)
